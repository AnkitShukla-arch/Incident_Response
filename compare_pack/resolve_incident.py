...
def ensure_schema(conn) -> None:
    """Apply the schema migrations the write-back path relies on.

    1. Make root_cause nullable: seed data always has a root_cause, but
       incidents resolved through the UI store the LLM's freeform suggestion as
       `resolution` and don't reliably know a separate root cause.
    2. Add the postmortem_doc column (postmortem.py writes the generated
       postmortem document here right after the write-back).

    Both are idempotent, so it's safe to run on every startup (metadata-only
    changes on first run, no-ops after).
    """
    conn.execute("ALTER TABLE incidents ALTER COLUMN root_cause DROP NOT NULL")
    conn.execute("ALTER TABLE incidents ADD COLUMN IF NOT EXISTS postmortem_doc TEXT")
    conn.commit()
